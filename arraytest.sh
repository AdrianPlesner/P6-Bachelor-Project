#!/usr/bin/env bash
#SBATCH --job-name MLA-params # CHANGE this to a name of your choice
...
srun singularity exec --nv -B src:/src -B results:/results -B data/"$SLURM_ARRAY_TASK_ID":/data  mxnet_21.02-py3.sif python /src/main.py results/MLA/"${path[$SLURM_ARRAY_TASK_ID]}"/metadata.json
