...
declare -a path=("DF-factors" "DF-glayers" "DF-gunits" "DF-llayers" "DF-lunits")
declare -a param=("factors" "global_layers" "global_units" "local_layers" "local_units")
declare -a step=(5 1 5 1 5)
echo "this is job $SLURM_ARRAY_JOB_ID with task $SLURM_ARRAY_TASK_ID"
srun singularity exec --nv -B src:/src -B results:/results -B data/"$SLURM_ARRAY_TASK_ID":/data  mxnet_21.02-py3.sif python /src/main.py results/Pems/"${path[$SLURM_ARRAY_TASK_ID]}"/metadata.json "${param[$SLURM_ARRAY_TASK_ID]}" "${step[$SLURM_ARRAY_TASK_ID]}"
